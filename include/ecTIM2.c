...
void TIM_period_us(TIM_TypeDef *TIMx, uint32_t usec){   
	...
	// 0.01ms(100kHz, ARR = 1) to 40,000,000 msec (ARR = 0xFFFF FFFF) for 2,5
	...
	if (TIMx == TIM2 || TIMx == TIM5){
		...
		PSCval = Sys_CLK/1000000;						// 84 or 16	--> f_cnt = 1kHz
		ARRval = Sys_CLK/PSCval/1000000 * usec;						// ARRval= 1*usec
		TIMx->PSC = PSCval -1;
		TIMx->ARR = ARRval - 1;
	}			
...
void TIM_CountDIR(TIM_TypeDef* TIMx, int direction){
		TIMx->CR1 &= ~(1<<4);
		TIMx->CR1 |= (direction<<4);
}
