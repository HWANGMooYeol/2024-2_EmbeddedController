...
// 4. Configure Timer Output mode as PWM
	...
	if(chN == 1){
		...
		TIMx->CCMR1 |= TIM_CCMR2_OC3M_1 | TIM_CCMR2_OC3M_2;	
		TIMx->CCMR1 |= TIM_CCMR2_OC3PE;
		TIMx->CCR3   = ccVal;
		TIMx->CCER  &= ~TIM_CCER_CC3P;
		TIMx->CCER  |=  TIM_CCER_CC3E;
	}
	else if(chN == 4){
		TIMx->CCMR2 &= ~TIM_CCMR2_OC4M;                     // Clear ouput compare mode bits for channel 2
...
// 3. Configure prescaler PSC
	...
	uint32_t value = fclk*pulse_width_ms - 1;					// pulse_width_ms *fclk - 1;
...
// 0. Match TIMx from  Port and Pin 	
	...
	uint32_t arr_value = TIMx->ARR;
	uint32_t value = (arr_value+1)*duty + 1;    				//CCR // (ARR+1)*dutyRatio + 1               
