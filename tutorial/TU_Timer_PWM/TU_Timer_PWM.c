...
int main(void) { 
	...
	GPIOA->AFR[0] &= ~(0b1111<<4 *LED_PIN);      // Clear
	GPIOA->AFR[0] |= (0b0001<<4 * LED_PIN);     // AF1 at PA5 = TIM2_CH1 (p.150)
	
	// TIMER: PWM setting
	RCC->APB1ENR |=  1;        				// Enable TIMER clock
	
	TIMx->CR1 &=  ~(1<<4);				              		// Direction Up-count
	
	TIMx->PSC =  0b1101000111;   //839				      // Set Timer CLK = 100kHz : (PSC + 1) = 84MHz/100kHz --> PSC = ?
	
	TIMx->ARR = 	0b1100011;			//99					        // Auto-reload: Upcounting (0...ARR). 
	...
	TIMx->CCR1 =    50;   								//Duty ratio 50%		// Output Compare Register for channel 1 	
	
	TIMx->CCER &= ~TIM_CCER_CC1P;    			// select output polarity: active high	
	TIMx->CCER |= 	1;											// Enable output for ch1
